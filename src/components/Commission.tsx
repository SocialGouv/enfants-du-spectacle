  ...
  return (
    ...
    >
      ...
      <div>
        <b>
          {dossier.enfants
            ? dossier.enfants.length
            : dossier._count?.enfants
            ? dossier._count.enfants
            : 0}
        </b>
        &nbsp;enfants
...
const Commission: React.FC<Props> = ({ commission }) => {
  ...
  return (
    ...
    >
      ...
      >
        {session?.dbUser.role !== "MEDECIN" && <div>Etats</div>}
        <div>Dossier</div>
        <div>Société</div>
        <div>Enfants</div>
        <div>{`${
          session?.dbUser.role !== "MEDECIN" ? "Suivi par" : "MÉDECIN"
        }`}</div>
        <div>Catégorie</div>
            ...
            >
              <Dossier
                ...
                commentsInfo={{
                  ...
                  notificationsProject: dossier.comments
                    ? dossier.comments.filter(
                        (com) =>
                          com.source === "SOCIETE_PROD" &&
                          com.seen !== true &&
                          com.enfantId === null
                      ).length
                    : 0,
                  notificationsChildren: dossier.comments
                    ? dossier.comments.filter(
                        (com) =>
                          com.source === "SOCIETE_PROD" &&
                          com.seen !== true &&
                    ...
                    : 0,
                  newPiecesEnfant: dossier.enfants
                    ? dossier.enfants.flatMap((enf: any) =>
                        (enf.piecesDossier || []).filter(
                          (piece: any) => piece.statut === null
                        )
                      ).length
                    : 0,
                  newPiecesDossier: dossier.piecesDossier
                    ? dossier.piecesDossier.filter(
                        (p: any) => p.statut === null
                      ).length
                    : 0,
                }}
