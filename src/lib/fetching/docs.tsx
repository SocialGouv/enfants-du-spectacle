...
const uploadDoc = async (data: FormData, dossierId: string, enfantId: string, typeJustif?: JustificatifEnfant): Promise<{filePath: string}> => {
    ...
    const url = `https://formulaire-enfants-du-spectacle-preprod.dev.fabrique.social.gouv.fr/api/sync/inc/upload?id=${dossierId}&typeJustif=${typeJustif}&enfantId=${enfantId}`
