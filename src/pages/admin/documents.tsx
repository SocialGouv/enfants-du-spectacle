...
const DocumentsAdmin: React.FC<Props> = () => {
  ...
  const updateFormData = (categorie: string, field: 'nom' | 'description', value: string) => {
    setFormData(prev => ({
      ...prev,
      [categorie]: {
        ...prev[categorie],
      ...
      <Container>
        <Row>
          <Col n="12">
            <h1>Gestion des documents importants</h1>
            <p>
              Ces documents seront accessibles publiquement depuis l'application
              formulaire pour les sociétés de production.
            </p>
              ...
              {Object.entries(CATEGORIES).map(([categoryKey, categoryLabel]) => {
                ...
                return (
                  <div key={categoryKey} style={{ 
                    ...
                    <h3 style={{ marginTop: 0, marginBottom: "1rem", color: "#0047AB" }}>
                      {categoryLabel}
                    </h3>
                    ...
                    {existingDocument && (
                      <div style={{ 
                        ...
                        <h4 style={{ margin: 0, marginBottom: "0.5rem", color: "#0056b3" }}>
                          Document actuel
                        </h4>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                          <div style={{ flex: 1 }}>
                            <p style={{ margin: 0, fontWeight: "bold" }}>{existingDocument.nom}</p>
                            ...
                            <p style={{ margin: "0.25rem 0", fontSize: "0.85em", color: "#888" }}>
                              Fichier: {existingDocument.originalName} ({formatFileSize(existingDocument.size)})
                            </p>
                            <p style={{ margin: "0.25rem 0", fontSize: "0.85em", color: "#888" }}>
                              Uploadé le {new Date(existingDocument.createdAt).toLocaleDateString("fr-FR")} par{" "}
                              {existingDocument.user.prenom || existingDocument.user.nom
                                ? `${existingDocument.user.prenom || ""} ${existingDocument.user.nom || ""}`
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                      <div>
                        <label htmlFor={`nom-${categoryKey}`} style={{ display: "block", marginBottom: "0.5rem", fontWeight: "bold" }}>
                          {existingDocument ? "Remplacer par un nouveau document" : "Nom du document *"}
                        </label>
                        <input
                          id={`nom-${categoryKey}`}
                          type="text"
                          value={formData[categoryKey]?.nom || ""}
                          onChange={(e) => updateFormData(categoryKey, 'nom', e.target.value)}
                      ...
                      <div>
                        <label htmlFor={`description-${categoryKey}`} style={{ display: "block", marginBottom: "0.5rem", fontWeight: "bold" }}>
                          Description (optionnelle)
                        </label>
                        <textarea
                          id={`description-${categoryKey}`}
                          value={formData[categoryKey]?.description || ""}
                          onChange={(e) => updateFormData(categoryKey, 'description', e.target.value)}
